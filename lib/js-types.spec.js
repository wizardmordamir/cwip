...
describe('arrays', () => {
  ...
  describe('stringIncludesAny', () => {
    it('should find string', () => {
      expect(stringIncludesAny(['ats'], 'cats')).toBe(true);
    });

    it('should not find string', () => {
      expect(stringIncludesAny(['atS'], 'cats')).toBe(false);
    });

    it('should find string insensitive', () => {
      expect(stringIncludesAny(['atS'], 'cats', true)).toBe(true);
    });

    it('should not find string insensitive', () => {
      expect(stringIncludesAny(['atSa'], 'cats', true)).toBe(false);
    });
  });
  ...
  describe('isObjectNotArray', () => {
    it('should be object', () => {
      ...
      expect(vals.map(isObjectNotArray)).toEqual(Array(vals.length).fill(true));
    });
    it('should not be object', () => {
      const vals = [null, undefined, false, [], 'a', 1, '', 0, true];
      expect(vals.map(isObjectNotArray)).toEqual(Array(vals.length).fill(false));
    });
  });
  ...
  describe('isEmptyDeep', () => {
    it('should find empty', () => {
      ...
      expect(truthy.map(isEmptyDeep)).toEqual(Array(truthy.length).fill(true));
      const falsy = [{ a: 1 }, { a: { b: '' } }, { a: { b: [''] } }];
      expect(falsy.map(isEmptyDeep)).toEqual(Array(falsy.length).fill(false));
    });
    ...
    it('should find not empty', () => {
      ...
      expect(vals.map(isEmptyDeep)).toEqual(Array(vals.length).fill(false));
    });
  });
});
