/**
 * Generic runtime boot-smoke harness for the cross-repo promotion gate.
 *
 * The gate's `bun run build` proves an integration worktree TYPE-checks and BUNDLES,
 * but a green build can still fail at RUNTIME: a module that throws on import, a
 * missing export that only blows up when the server actually boots, a bad dynamic
 * import. This module adds the second, independent signal: boot a consumer's server
 * against an ISOLATED state dir + a unique port, wait for it to answer a health
 * endpoint, then tear it down. A build that compiles but won't run can never promote
 * `main`.
 *
 * Split into a PURE plan layer (`planSmoke`/`smokeEnv` — fully unit-testable, no
 * spawning) and an impure `runBootSmoke` that spawns + polls. The spawn + health-poll
 * + log-capture is delegated to `startTestServer` (the same primitive the functional
 * testkit uses), injected so unit tests drive it without a real process. Bounded by
 * `timeoutMs` and side-effect-isolated (its own throwaway home dir, removed after),
 * so the watchdog can run it inline every cycle without risk.
 *
 * App-specific presets (`rubatoSmokeSpec`, `caSmokeSpec`) live in each app repo —
 * only the generic, app-agnostic machinery lives here.
 */
import { mkdir, rm } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { type StartTestServerOptions, startTestServer } from './startTestServer';
 ...
 * so the smoke never reads or writes the real app home dir.
 */
