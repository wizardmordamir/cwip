...
- **Auto-tiering (model + think) — classify-on-eligible (shipped):** a task's `model` defaults to the `auto` sentinel (also: unset/null — see `needsTiering`). The moment it becomes ELIGIBLE — assessed by the drainer/fleet just before a claim, never on a recurring sweep — the engine picks the optimal `{model, think}` from its title/body/repo/scope (`classifyTier`: heavy → opus/max, light → sonnet/medium, ambiguous → conservative opus/high) and WRITES it back as an EXPLICIT alias. That explicit value IS the "already-assessed" marker, so re-assessment is **idempotent by construction** — no flag, no re-scan. An owner-set alias is respected and never touched; the owner forces a re-assessment by re-setting the model to `auto`. Lazy: only a task that actually runs gets assessed. Heuristics are pure/free; an ambiguous task can be refined by injecting a cheap-model `classify` callback at the consumer (cwip stays runtime-free). Wired into `claim`/`claimNext` (the fleet claim) + `autoTierEligible`/`autoTierTask`; an `auto`/untagged task matches every tier filter until assessed, so it routes to the right fleet the instant its tier is chosen.
- **Triage (opt-in, legacy/superseded for model+think):** a task with blank model/think enters `pending_triage`; a cheap (Haiku) triage call grades complexity and assigns model/think, then → `ready`. Globally toggleable; the triage spend itself is **budget-capped**. (The model/think decision is now handled by the always-on, deterministic auto-tiering above; `pending_triage` remains for the complexity/epic grade.)
